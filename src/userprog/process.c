...
pid_t process_execute(const char* file_name) {
  ...
  // Copy file name only up the the first space to ensure process name is correct
  size_t len = strcspn(file_name, " " ) + 1;
  char *process_name = malloc(len);
  strlcpy(process_name, file_name, len);
